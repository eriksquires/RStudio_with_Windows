...
much faster than compiling source packages from CRAN, not to mention
avoids many dev library dependency sissues. Duckdb for instance can take
an hour or more to install from CRAN. [For more information about using
the Posit binary repos go
...
**Protip:** if you are having dependency issues with a packages, try
installing the Ubuntu repo’s package first, and then re-installing your
user library version. This will help because the repo builders do an
excellent job of installing all the OS package dependencies. You can
